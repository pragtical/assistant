...
---Abstract byte transport used by assistant backends to communicate over
---either stdio or tcp.
---@class assistant.transport
---Selected backend kind.
---@field public kind assistant.transport.kind
---True once the transport has completed startup and is ready to exchange bytes.
---@field public startup_ok boolean
---Set when transport startup fails permanently. Once this is non-nil the
---backend should stop retrying and treat startup as failed.
...
---Read up to `amount` bytes from the main transport channel.
...
---Write raw bytes to the main transport channel.
...
---Returns true once the transport is fully connected and able to exchange bytes.
---@return boolean
