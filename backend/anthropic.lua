    ...
    local function apply_stream_event(event_type, data, emit_text)
      ...
      if event_type == "content_block_delta" then
        ...
        if delta.type == "text_delta" then
          ...
          local append_text = stream_text_delta(partial_text, text, text_delta_state)
          if append_text ~= "" then
            if block then block.text = (block.text or "") .. append_text end
            table.insert(chunks, append_text)
            partial_text = partial_text .. append_text
          end
      ...
      on_done = function(ok, err, _, info)
        ...
        if ok and not is_http_error(info) then
          ...
          local final_should_auto_implement = final_is_completed_plan
            and not calls_include_tool(agent, tool_calls, "implement_plan")
          ...
          elseif has_streamed_tool_calls and final_is_completed_plan then
            if calls_include_tool(agent, tool_calls, "implement_plan") then
              ...
              finish(final_text, info, usage, { auto_implement_plan = true })
            end
          elseif has_streamed_tool_calls and #tool_calls > 0 then
            ...
            partial_text = ""
            request_tool_approval(tool_calls, round or 0, true)
          elseif tool_calls and #tool_calls > 0 and not final_should_auto_implement then
            compact_after_done()
            chunks = {}
            partial_text = ""
            request_tool_approval(tool_calls, round or 0, true)
