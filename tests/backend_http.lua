...
test.describe("assistant http backend", function()
  ...
  test.it("records local tool activity in the rendered transcript", function()
    ...
    test.equal(markdown:find("## Activity", 1, true) ~= nil, true)
    test.equal(markdown:find("Running command", 1, true) ~= nil, true)
    test.equal(markdown:find("Command: `make test`", 1, true) ~= nil, true)
    test.equal(markdown:find("All tests passed", 1, true) ~= nil, true)
  end)
  ...
  test.it("limits file read results in activity when verbose tool calling is disabled", function()
    ...
    restore_background_threads()
    local markdown = conversation:to_markdown()
    test.equal(markdown:find("1:first", 1, true) ~= nil, true)
    test.equal(markdown:find("3:third", 1, true) ~= nil, true)
    test.equal(markdown:find("4:fourth", 1, true), nil)
    test.equal(markdown:find("truncated after 3 lines", 1, true) ~= nil, true)
